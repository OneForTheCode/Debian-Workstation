...
prompt_command_hook()
{
	...
	*)
	...
	while [ "${#elements[@]}" -gt 1 ]; do
		if [ "$FIRSTITEM" == 0 ]; then
			FIRSTITEM=1
			tmp_separator_padding_left=$separator_padding_left
			tmp_separator_padding_right=$separator_padding_right
			tmp_segment_padding=$segment_padding
			separator_padding_left=''
			separator_padding_right=''
			segment_padding=''
		else
			separator_padding_left=$tmp_separator_padding_left
			separator_padding_right=$tmp_separator_padding_right
			segment_padding=$tmp_segment_padding
		fi
